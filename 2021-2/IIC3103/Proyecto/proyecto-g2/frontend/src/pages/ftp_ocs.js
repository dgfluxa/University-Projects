...
import OCTable from '../components/oc_table';
...
function FTPOCs() {
	...
	useEffect(() => {
		OCsService.getGeneralOCs().then((data) => {
			setOCs(data);
			setLoaded(true);
		});
	}, []);
									...
									<div style={{display: 'flex', marginTop: '2%', justifyContent: 'center'}}>
										<Paper style={{width: '20%', minWidth: 300}}>
											<Table size='small'>
												<TableBody>
													<TableRow>
														<TableCell style={{color: 'orange'}}>Pendientes:</TableCell>
														<TableCell >{ocs.estadisticas.pendientes}</TableCell>
													</TableRow>
													<TableRow>
